CREATE TABLE mf_source_resume_states (
  pipeline_id text PRIMARY KEY,
  source_cursor blob NOT NULL,
  paused integer NOT NULL DEFAULT 0 CHECK (paused IN (0, 1))
);

CREATE TABLE mf_segment_outputs (
...
CREATE INDEX mf_segment_outputs_origin_idx
  ON mf_segment_outputs (pipeline_id, segment_id, origin_record_id, id);

CREATE TABLE mf_terminal_records (
  id integer PRIMARY KEY AUTOINCREMENT,
  pipeline_id text NOT NULL,
  origin_record_id text NOT NULL,
  record_id text NOT NULL,
...
CREATE INDEX mf_terminal_records_origin_idx
  ON mf_terminal_records (pipeline_id, origin_record_id, id);

CREATE TABLE mf_source_records (
  id integer PRIMARY KEY AUTOINCREMENT,
  pipeline_id text NOT NULL,
  origin_record_id text NOT NULL,
  record_id text NOT NULL,
...
CREATE INDEX mf_source_records_pipeline_idx
  ON mf_source_records (pipeline_id, id);

CREATE TABLE mf_pending_segment_work (
  id integer PRIMARY KEY AUTOINCREMENT,
  pipeline_id text NOT NULL,
...
CREATE INDEX mf_pending_segment_work_pipeline_idx
  ON mf_pending_segment_work (pipeline_id, id);

CREATE TABLE mf_segment_progress (
  pipeline_id text NOT NULL,
  segment_id text NOT NULL,
  record_id text NOT NULL,
  attempt_id integer NOT NULL,
...
CREATE TABLE mf_deterministic_segment_results (
  pipeline_id text NOT NULL,
  segment_id text NOT NULL,
